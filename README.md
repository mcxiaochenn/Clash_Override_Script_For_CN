...
| 类型 | 版本格式 | 说明 | 更新频率 |
|------|----------|------|----------|
| 稳定版 | `vYYYY.MM.DD.NB` | 经过验证的正式版本（Latest） | 手动发布 |
| 每日版 | `vYYYY.MM.DD.NB` | 包含最新功能的自动构建版（Pre-release） | 每日北京时间 8:00 |

> `NB` 为当日第几次发布，从 01 开始递增，新的一天重置。使用 Asia/Shanghai 时区。命名格式统一，仅 Latest / Pre-release 标签不同。
